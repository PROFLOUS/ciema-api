...
class StatisticsRepository {
  ...
  async GetRevenueByStaff({
    ...
    const data = await Order.findAll({
      ...
      include: [
        {
          model: Staff,
          attributes: ["id", "firstName", "lastName", "cinema_id"],
        },
      ],
      attributes: [
  ...
  async GetRevenueByPromotionLine({
    ...
    const data = await PromotionResult.findAll({
      ...
      attributes: [
        ...
        [sequelize.col("PromotionLine.PromotionDetail.max_money_reduction"), "discount"],
      ],
    });
    return data;
  }
  ...
  async GetRatioDashboardInMonth(month) {
    ...
    await Promise.all([
      ...
      ratio_revenue = Math.round( (data_revenue_current - data_revenue_previous) / data_revenue_previous * 100);
      ...
      ratio_new_user = Math.round( (data_new_user_current - data_new_user_previous) / data_new_user_previous * 100);
      ...
      ratio_ticket = Math.round( (data_ticket_current - data_ticket_previous) / data_ticket_previous * 100);
      ...
      ratio_order_refund = Math.round( (data_order_refund_current - data_order_refund_previous) / data_order_refund_previous * 100);
